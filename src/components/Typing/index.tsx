...
const getClassName = (fixedWidth: boolean, status: TypingStatus | undefined) => {
...
type TypingProps = React.HTMLAttributes<HTMLDivElement> & {
  str?: string
  children?: string
  speed?: number
  preDelay?: number
...
const Typing = ({
  ...
  const disassembled = useMemo(() =>
    source.split('').map(ch => Hangul.disassemble(ch))
  , [source]);
  ...
  const className = getClassName(!!fixedWidth, status);
